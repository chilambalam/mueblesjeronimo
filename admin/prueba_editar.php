...
if (isset($_POST['Guardar']))
{
	...
	{ 
		if ( (strpos($tipo_archivo4 , "jpeg") || strpos($tipo_archivo4 , "png") || strpos($tipo_archivo4 , "gif")) && ($tamano_imagen < 2097152) ) 
		{
			...
			mysql_select_db($database_con_admin, $con_admin);
			$qry44 = "UPDATE producto SET titulo='$titulo',descripcion='$descripcion', imagen='$nombre_archivo4' WHERE id_producto='$id_s'";	
		...
		{  
			...
			echo"<script language='javascript'>window.location='prueba_editar.php?menu=".$menu."&seccion=".$seccion."'</script>";					
		}
	}
	else
	{
		mysql_select_db($database_con_admin, $con_admin);
		$qryss = "UPDATE producto SET titulo='$titulo',subtitulo='$subtitulo', url='$url' WHERE id_producto='$id_s'";
		mysql_query($qryss, $con_admin) or die(mysql_error());
		echo "<SCRIPT LANGUAJE='javascript'>window.location='prueba2.php?menu=".$menu."'</SCRIPT>";		
	}	
}
?>